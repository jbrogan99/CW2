...
var weightingResponse; // global variables
var degreesData;
var assessmentData;
...
if (submitAllButton) {
  submitAllButton.addEventListener("click", function (evt) {
    evt.preventDefault();
    var errorMessage = document.getElementById("fillInFieldModuleDegree"); // getting values / ID
    ...
    var numberFromString = /\d+/; // reg ex numbers only

    var assessment_selectValue =
      document.getElementById("assessment_select").value;
    percentageValue = assessment_selectValue.match(numberFromString); // get numbers from stirng
    percentage = parseInt(percentageValue); // convert from string to integer

    if (degree_select1) {
      // if element exists
      var degree_select1Value = document.getElementById("degree_select1").value;
    }
    if (degree_select2) {
      var degree_select2Value = document.getElementById("degree_select2").value;
    }
...
// this fucntion extracts the module name and adds it to option tag
...
fetch(
  ...
  .then((response) => {
    ...
    addAnotherDegreesDropdown("", response, true); // add plus sign to first drop down option
    count = 1;

    var plusSign = document.getElementById("plusSign");
    plusSign.addEventListener("click", function (e) {
      if (count < 3) {
        // maximum of 3 degrees to one tr
        addAnotherDegreesDropdown(count, degreesData, false); // dont add plus sign to rest of drop down options
        count++;
      }
    ...
    minusSign.addEventListener("click", function (e) {
      ...
      if (count > 1) {
        count--;
        removeInputBox(count); // removing option tag when minus button clicked
      }
    });
  });

function removeInputBox(count) {
  // adds unique ids for each element
  console.log(count);
...
function addAnotherDegreesDropdown(count, degrees, addButtons) {
  // dynamcally add containers and inputs to manipulate the dom
...
function addAnotherAssessmentDropdown(counter, assessments, addButtons) {
  ...
  newAssessmentContainer.setAttribute("id", "newAssessmentContainer" + counter);
